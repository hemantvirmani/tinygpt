...
## Phase 10: Instruction Fine-Tuning (May 2026)

Pretraining taught TinyGPT how language works. It could complete sentences, continue paragraphs, and generate text that reads as coherent. What it couldn't do was be *helpful*. Asked "What is photosynthesis?", the pretrained model responded with a list of follow-up questions. It had learned the structure of educational text — which often contains question lists — without learning that a human was asking for an answer.

Instruction fine-tuning is the step that closes this gap. Rather than predicting the next token in a stream of web text, the model is trained on examples of the form: here is an instruction, here is the expected response. Over thousands of such examples, the model learns a new behavior: when it sees an instruction, answer it directly.

This is the same technique that turns a language model into something like ChatGPT — though at much smaller scale and without the RLHF (reinforcement learning from human feedback) step that further polishes the output.

### The Template

Every training example follows the same format:
...
The model learns to generate the Response section given an Instruction. At inference time, you provide the instruction and let the model complete the response.

### First Attempt: Dolly 15K — and Why It Failed

The first fine-tuning dataset tried was Databricks' Dolly 15K — 15,000 instruction-response pairs written by Databricks employees. It's a curated, high-quality dataset. The problem: 15,000 examples is thin for a 163M parameter model.

The ratio matters here just as it does in pretraining. With 163M parameters and only 13,500 training examples, the model had more capacity than data to constrain it. Within a few hundred steps, it began to memorize the training set rather than generalize. Validation loss fell to 2.62 at step 500, then started climbing — reaching 3.09 by step 800. The model was getting *better* at reciting training data and *worse* at handling new examples.

The lesson from pretraining repeated itself on a smaller scale: the ceiling is set by the data.

### Switching to Alpaca Cleaned — 52K Examples

The switch was to [yahma/alpaca-cleaned](https://huggingface.co/datasets/yahma/alpaca-cleaned) — a cleaned version of Stanford Alpaca's 52,000 instruction-response pairs, generated by GPT-3.5 using a self-instruct approach. Same three fields: `instruction`, `input` (optional context), `output` (expected response). Same template.

52K vs 15K is a meaningful difference. More examples means the model has to learn *general* patterns rather than memorize specifics. Whether it's enough for a 163M parameter model is a question fine-tuning answers empirically.

### Fine-Tuning Setup

| Setting | Value |
| --- | --- |
| Base weights | TinyGPT pretrained (val loss 2.8368) |
| Dataset | Alpaca Cleaned (52K examples) |
| Dropout | **0.1** (was 0.0 during pretraining) |
| Learning rate | 1e-4 with linear warmup + cosine decay |
| Effective batch size | 64 (4 micro-batch × 16 accumulation steps) |
| Max steps | 5,000 |
| Loss masking | Padding tokens masked — only Response tokens train the model |
| Checkpoint strategy | Save-on-best: one file, overwritten only when val loss improves |

Two decisions worth explaining:

**Dropout = 0.1.** During pretraining, dropout was set to 0.0 — standard practice when training on billions of tokens, because the model won't overfit data it's only seen once. Fine-tuning is different: the model sees the same 52K examples repeatedly. Without dropout, the train/val gap ballooned to 0.80+ (train 1.5, val 2.3) after 2,000 steps. With dropout = 0.1, the gap stayed at 0.40–0.50 — a much healthier training signal.

**Loss masking.** Only the `### Response:` tokens contribute to the loss. The instruction and input tokens are masked out. This ensures the model is learning *to answer*, not just predicting the template structure. If you trained on the full sequence, the model would spend most of its gradient signal learning to reproduce `### Instruction:` formatting rather than content.

**Save-on-best.** Earlier periodic saves (every 500 steps) missed the best checkpoint — the best val loss would fall between save points. The fix: save only when val loss improves, overwriting a single file. At the end of training, the saved file is guaranteed to be the best the model ever achieved.

### The Training Run

| Step | Train Loss | Val Loss | Note |
| --- | --- | --- | --- |
| 1 | 5.7987 | 2.6124 | Starting from pretrained weights |
| 100 | 2.2636 | 1.9667 | Fast format acquisition |
| 500 | 1.8706 | 1.9092 | |
| 2,200 | 1.4700 | 1.9013 | |
| 2,600 | 1.4906 | 1.8627 | |
| **3,600** | **1.3651** | **1.8405** | **Best checkpoint** |
| 5,000 | 1.3817 | 2.0713 | Val loss rising — past peak |

The fastest learning happened in the first 100 steps — the model acquired the instruction-response format almost immediately. The val loss dropped from 2.61 to 1.97 (a drop of 0.64 nats) in the first 100 steps alone. Everything after that was slower refinement.

The best val loss of **1.8405** arrived at step 3,600. After that, training loss kept declining (the model was memorizing more of the training set), but val loss started drifting upward — the telltale sign of overfitting. Without the save-on-best strategy, this checkpoint would have been missed.

### Before and After

The proof is in what the model actually outputs.

#### Before fine-tuning — "What is photosynthesis?"

> *Why would you want to know what is photosynthesis? What are the two most important parts of photosynthesis? Why is photosynthesis important? What is the reason why photosynthesis is important?...*

The model generates more questions. It had learned the pattern of educational text — which often lists related questions — without understanding that it was expected to answer.

#### After fine-tuning — "What is photosynthesis?"
...
The format shift is clear. The model now reads an instruction as something to answer, not a prompt to continue.

#### Before — "Explain the water cycle in simple terms."

> *1. What is the amount of water in the environment that can be found in the streams and lakes? 2. Why is water important for animals in the streams?...*

#### After — "Explain the water cycle in simple terms."

> *The water cycle, also known as the hydrologic cycle, is the continuous movement of water on, above, and below the surface of the earth. It starts with the evaporation of water from the ground and rises into the atmosphere through the process of precipitation...*

#### After — "Who was the first emperor of Rome?"

> *The first Emperor of Rome was Julius Caesar.*

The format is right — a direct answer, no question-list. The fact is wrong (Augustus, not Julius Caesar). This is a 163M parameter model trained on a filtered dataset: it learned to follow instructions, but its factual recall is limited and sometimes incorrect. Julius Caesar was famous and associated with Rome's transition to empire in the training data, even though Augustus was technically the first emperor. At this scale, factual accuracy requires either a larger model, retrieval augmentation, or explicit fact-checking.

One other artifact: `<|endoftext|>` tokens appeared mid-generation. The model learned to produce them as part of the training data format, but the inference code didn't stop when it saw one — it kept generating past it. A small fix in `generate_text()` (stop when EOT token is produced) would clean this up.

### What Fine-Tuning Taught

1. **Format is learned very fast.** 100 steps on 52K examples was enough to shift the model's behavior from question-generation to answer-generation. The weight updates for "respond to instructions" are shallow and quick — the underlying language model doesn't need to relearn how language works.

2. **Dropout is essential for fine-tuning, optional for pretraining.** The difference between dropout=0.0 and dropout=0.1 during fine-tuning was a train/val gap of 0.80+ vs. 0.40–0.50. Without it, the model overfits its training examples visibly within 2,000 steps.

3. **Save-on-best is mandatory.** The best val loss (1.8405 at step 3,600) fell between periodic save points. If saves had only happened at step 3,500 or 4,000, the best checkpoint would have been lost. When the loss curve is noisy and the best could fall anywhere, periodic saves are insufficient.

4. **Factual accuracy is a model capacity problem.** The model learned the instruction format. It did not magically acquire better knowledge than it had after pretraining. A 163M parameter model has limited factual memory; errors like "Julius Caesar" vs. "Augustus" come with the territory. Fixing this requires a larger base model, retrieval, or targeted data curation.

5. **The Chinchilla lesson applies here too.** 163M parameters on 52K examples is data-thin. With more instruction data — or a curated, higher-quality set — the overfitting pressure would be lower. The next improvement to reach for is either more data, LoRA fine-tuning on a larger base, or both.
